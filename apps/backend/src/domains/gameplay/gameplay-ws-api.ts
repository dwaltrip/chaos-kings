import { GAMEPLAY_DOMAIN, type GameplayServerInbound } from '@common/types/gameplay';
import {
  queueMove,
  cancelQueuedMoves,
  addUserToGame,
...
function handleGameplayMessage(msg: GameplayServerInbound, wsActions: any): void {
      ...
      if (user) {
        ...
        if (gameId) {
          ...
          if (gameServer) {
            ...
            console.log(`Player ${userId} joined gameplay room for game ${gameId}`);
          } else {
            console.error(
...
export { handleGameplayMessage, addUserToGame, removeUserFromGame, getUserGame };
