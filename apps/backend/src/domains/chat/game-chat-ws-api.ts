import { GAME_CHAT_DOMAIN, type GameChatServerInbound } from '@common/types/game-chat';
import { createGameChatEffects } from '@/game-chat/ws-effects';
import { postMessage } from '@/game-chat/actions/post-message-action';

function handleGameChatMessage(msg: GameChatServerInbound, wsActions: any): void {
  ...
  switch (msg.type) {
    ...
    case 'post-message': {
      ...
      void postMessage(Number(user.id), user.username, room, content, effects);
      break;
    }
    default:
      break;
...
export { GAME_CHAT_DOMAIN, handleGameChatMessage };
