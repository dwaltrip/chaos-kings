...
const genMap = (size: Size2d, numPlayers = 2, minGeneralDistance = 4, seed = 42) => {
...
describe('generateGameMapV2', () => {
  ...
  it('should enforce minimum general distance', () => {
    ...
    for (let i = 0; i < result.generals.length; i++) {
      for (let j = i + 1; j < result.generals.length; j++) {
        ...
        const distance =
          Math.abs(gen1.coord.x - gen2.coord.x) + Math.abs(gen1.coord.y - gen2.coord.y);
  ...
  it('should be deterministic with same seed', () => {
    ...
    for (let i = 0; i < result1.generals.length; i++) {
      ...
      expect(result1.generals[i].playerIndex).toBe(result2.generals[i].playerIndex);
    }
  });
