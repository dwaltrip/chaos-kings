...
function generateTerrain(
  ...
  while (obstaclesPlaced < targetObstacles && totalFailures < MAX_TOTAL_FAILURES) {
...
function validateParameters(width: number, height: number, targetDensity: number): void {
  if (width < MIN_GRID_SIZE || width > MAX_GRID_SIZE) {
    throw new Error(`Width must be between ${MIN_GRID_SIZE} and ${MAX_GRID_SIZE}`);
  }

  if (height < MIN_GRID_SIZE || height > MAX_GRID_SIZE) {
    throw new Error(`Height must be between ${MIN_GRID_SIZE} and ${MAX_GRID_SIZE}`);
  }
