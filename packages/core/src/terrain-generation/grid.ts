...
import { MIN_GRID_SIZE, MAX_GRID_SIZE } from '@core/terrain-generation/constants';
...
class Grid {
  ...
  constructor(width: number, height: number) {
    if (width < MIN_GRID_SIZE || width > MAX_GRID_SIZE) {
      throw new Error(`Grid width must be between ${MIN_GRID_SIZE} and ${MAX_GRID_SIZE}`);
    }
...
export { type Coord, type GridDimensions } from '@core/terrain-generation/types';
