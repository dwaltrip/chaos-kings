...
function extractBoxSize(
  ...
  if (sizes.length > 1) {
    console.warn(
      `useResizeObserver: Multiple ${name} entries detected, using first one`,
    );
  }
...
export function useResizeObserver<T extends HTMLElement = HTMLElement>(
  ...
  const handleResize = useCallback((entries: ResizeObserverEntry[]) => {
    requestAnimationFrame(() => {
      if (entries.length !== 1) {
        console.warn(
          'useResizeObserver: Expected exactly 1 entry, got',
          entries.length,
        );
        return;
      }
      ...
      const { borderBoxSize, contentBoxSize, devicePixelContentBoxSize } =
        entry;
  ...
  useEffect(() => {
    ...
    if (!window.ResizeObserver) {
      console.warn(
        'useResizeObserver: ResizeObserver is not supported in this browser',
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        'useResizeObserver: Failed to create ResizeObserver:',
        error,
      );
      return;
    }
