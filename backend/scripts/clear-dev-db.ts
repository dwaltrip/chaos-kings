...
async function clearDatabase() {
  ...
  const migrationTables = tableNames.filter(name => 
    name.includes('migration') || name.includes('schema_version')
  );
  
  // Clear each table in order (reverse dependency order)
  const tableOrder = ['game_players', 'games', 'users'];
  const tablesToClear = tableOrder.filter(table => 
    tableNames.includes(table) && !migrationTables.includes(table)
  );
  ...
  for (const tableName of tablesToClear) {
    try {
      ...
      const rowsDeleted = deleteResult.length > 0 ? deleteResult[0].numDeletedRows || 0 : 0;
  ...
  const remainingTables = tableNames.filter(name => 
    !tablesToClear.includes(name) && !migrationTables.includes(name)
  );
